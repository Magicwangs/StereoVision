...
@jit(nopython=True)
def speed(batchsize, le, referSubnet, targetSubnet, referSubnetBatch, targetSubnetBatch):
    for b in range(batchsize):
        referSubnetBatch[b] = referSubnet[le]
        targetSubnetBatch[b] = targetSubnet[le-b]
    return referSubnetBatch, targetSubnetBatch
...
#        output = np.nan_to_num(output)
    ...
    def fast_net_eval(self, pic, y_range, sess, netout, subNet_1, subNet_2, batchLeft, batchRight):
        ...
        d_out = np.zeros((self.picWidth, self.picLen, 221), dtype=np.float32)
    ...
    def stereomatch(self):
        ...
        for pic in picList:
            if pic.split('_')[1]=='10.png':
                ...
                t0 = time.clock()
                ## net eval
                disp, d_out, R_disp, R_d_out = self.fast_net_eval(pic, y_range, sess, netout, subNet_1, subNet_2, batchLeft, batchRight)
                t1 =  time.clock()-t0
                t0 = time.clock()
                ## left_right_check
                if right_dispDir is not None:
